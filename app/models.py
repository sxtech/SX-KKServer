...
class SVehiclePass(db.Model):
    """过车记录"""
    __bind_key__ = 'yc'
    __tablename__ = 'S_Vehicle_Pass'
    ...
    create_time = db.Column(
        db.DateTime, default=arrow.now('PRC').datetime.replace(tzinfo=None))
    video = db.Column(db.String(128), default='')

    def __init__(self, plate_no='-', plate_color=0, pass_time=None,
                 stat_code=0, vehicle_point_no=1, direction=1, status='1',
                 pic1='', pic2='', pic3='', pic4='', ip_addr='127.0.0.1',
                 serialno='', device_name='', uuid='', create_time=None,
                 video=''):
        self.plate_no = plate_no
        self.plate_color = plate_color
        self.pass_time = pass_time
    ...
    def __repr__(self):
        return '<S_Vehicle_Pass %r>' % self.id
...
class SDeviceState(db.Model):
    """相机设备状态"""
    __bind_key__ = 'yc'
    __tablename__ = 'S_Device_State'
    ...
    def __init__(self, ip_addr='', serialno='', device_name='', stat_code=0,
                 vehicle_point_no=1, direction=1, long=0.0, lat=0.0, ps='',
                 create_time='', last_modify='', update_flag=0, del_flag=0):
        self.ip_addr = ip_addr
        self.serialno = serialno
        self.device_name = device_name
        self.stat_code = stat_code
    ...
    def __repr__(self):
        return '<S_Device_State %r>' % self.id
